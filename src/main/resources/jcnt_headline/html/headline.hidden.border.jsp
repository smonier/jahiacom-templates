...
<c:set var="textContent">
    ...
    <template:include view="hidden.link">
        <template:param name="class" value="btn btn-outline-primary btn-circle px-4"/>
    </template:include>
...
<c:set var="isWistia" value="false"/>
<c:if test="${jcr:isNodeType(currentNode,'jcmix:headlineVideo')}">
    <c:set var="provider" value="${currentNode.properties.provider.string}"/>
    <c:if test="${provider eq 'wistia'}">
        <c:set var="isWistia" value="true"/>
    </c:if>
</c:if>
<div class="bg-white">
    <section>
        <div class="container">
            <div class="row border-gray p-5 d-flex align-items-center">
                <div class="container d-flex justify-content-center">
                    <div class="row ${verticalAlign}">
                        <c:choose>
                            <c:when test="${imagePosition eq 'left'}">
                                <div class="col-sm-6 d-none d-sm-none d-md-block">
                                    <div class="thumb ${isWistia?'': ' float-left'}">
                                            ${imageContent}
                                    </div>
                                </div>
                            ...
                            <c:otherwise>
                                ...
                                <div class="col-sm-6 d-none d-sm-none d-md-block">
                                    <div class="thumb  ${isWistia?'': ' float-right'}">
                                            ${imageContent}
                                    </div>
                                </div>
                            </c:otherwise>
